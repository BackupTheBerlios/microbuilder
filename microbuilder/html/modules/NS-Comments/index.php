<?php
// File: $Id: index.php,v 1.1 2004/03/01 10:09:06 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
        ...
        while(list($r_tid, $r_pid, $r_sid, $r_date, $r_name, $r_email, $r_url, $r_host_name, $r_subject, $r_comment, $r_score, $r_reason) = $result->fields) {
            ...
            if($r_score >= $thold) {
                ...
                if ($r_email) {
                    ...
                    echo "<br><font class=\"pn-normal\"> "._BY." </font><a class=\"pn-normal\" href=\"mailto:$r_email\">".pnVarPrepForDisplay($r_name)."</a> (".pnVarPrepForDisplay($r_email).")</font><font class=\"pn-sub\"> "._ON." ".pnVarPrepForDisplay($datetime)."</font>";
                } else {
                ...
                if (eregi("http://",$r_url)) { 
                	echo "<font class=\"pn-normal\"><a class=\"pn-normal\" href=\"$r_url\" target=\"window\">$r_url</a></font> "; 
                }
                echo "</td></tr><tr><td>";
                ...
                echo "</td></tr></table><br><br><div align=\"center\"><font class=\"pn-normal\">";
                if ($anonpost==1 OR pnUserLoggedIn()) {
                    echo " [ <a class=\"pn-normal\" href=\"modules.php?op=modload&amp;name=NS-Comments&amp;file=index&amp;req=Reply&amp;pid=$r_tid&amp;sid=$r_sid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
                } else {
        	...
        	$r_date=$result->UnixTimeStamp($r_date);
                ...
                if($r_score >= $thold) {
                    	...
                    	if ($r_email) {
                        	...
                        	echo "<font class=\"pn-normal\"><br>"._BY." <a class=\"pn-normal\" href=\"mailto:$r_email\">".pnVarPrepForDisplay($r_name)."</a> (".pnVarPrepForDisplay($r_email).")</font><font class=\"pn-sub\"> "._ON." ".pnVarPrepForDisplay($datetime)."</font>";
                     	} else {
                        	echo "<font class=\"pn-title\">".pnVarPrepForDisplay($r_subject)."</font> ";
                        	if (!$noshowscore && $modoption ) {
                            		echo "<font class=\"pn-normal\">("._SCORE." ".pnVarPrepForDisplay($r_score)."";
                        	...
                        	echo "<font class=\"pn-normal\"><br>"._BY." ".pnVarPrepForDisplay($r_name)." "._ON." ".pnVarPrepForDisplay($datetime)."</font>";
                    	}
                    	if ($r_name != $anonymous) {
				echo "<br><font class=\"pn-normal\">(<a class=\"pn-normal\" href=\"user.php?op=userinfo&amp;uname=$r_name&amp;module=NS-User\">"._USERINFO."</a> | <a class=\"pn-normal\" href=\"modules.php?op=modload&amp;name=Messages&amp;file=replypmsg&amp;send=1&amp;uname=$r_name\">"._SENDAMSG."</a>)</font> ";
            		}
                    	if (eregi("http://",$r_url)) {
            			echo "<font class=\"pn-normal\"><a class=\"pn-normal\" href=\"$r_url\" target=\"window\">".pnVarPrepForDisplay($r_url)."</a></font> ";
            		}
                    	echo "</td></tr><tr><td><font class=\"pn-normal\">";
                    	if(($maxcomments) && (strlen($r_comment) > $maxcomments)) {
                    		echo substr("$r_comment", 0, $maxcomments)."<br><br><a  class=\"pn-normal\" href=\"modules.php?op=modload&amp;name=NS-Comments&amp;file=index&amp;sid=$r_sid&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a>";
                    	} elseif(strlen($r_comment) > $commentlimit) {
                    		echo substr("$r_comment", 0, $commentlimit)."<br><br><b><a class=\"pn-normal\" href=\"modules.php?op=modload&amp;name=NS-Comments&amp;file=index&amp;sid=$r_sid&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a>";
                    	...
                    	echo "<div align=\"center\"><font class=\"pn-normal\">";
                    	if ($anonpost==1 OR pnUserLoggedIn()) {
	                        echo " [ <a class=\"pn-normal\"  href=\"modules.php?op=modload&amp;name=NS-Comments&amp;file=index&amp;req=Reply&amp;pid=$r_tid&amp;sid=$r_sid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
                    	} else {
                        	echo "[ "._NOANONCOMMENTS." ";
...
function DisplayBabies ($tid, $level=0, $dummy=0)
{
    ...
    while(list($r_tid, $r_pid, $r_sid, $r_date, $r_name, $r_email, $r_url, $r_host_name, $r_subject, $r_comment, $r_score, $r_reason) = $result->fields) {
        ...
        if (!eregi("[a-z0-9]",$r_subject)) { $r_subject = "["._NOSUBJECT."]"; }
        $datetime = ml_ftime(_DATETIMEBRIEF, GetUserTime($r_date));
        echo "<font class=\"pn-normal\"><a class=\"pn-normal\" href=\"modules.php?op=modload&amp;name=NS-Comments&amp;file=index&amp;req=showreply&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">$r_subject</a></font><font class=\"pn-normal\"> "._BY." ".pnVarPrepForDisplay($r_name)." "._ON." ".pnVarPrepForDisplay($datetime)."<br>";
    ...
    if (pnSecAuthAction(0, 'Stories::Story', "$info[aid]:$info[cattitle]:$info[sid]", ACCESS_COMMENT)) {
      ...
      while(list($tid, $spid, $sid, $name, $email, $url, $host_name, $subject, $comment, $score, $reason, $date) = $something->fields) {
        ...
        if ($email) {
            ...
            echo "<br>"._BY." <a class=\"pn-normal\" href=\"mailto:$email\">".pnVarPrepForDisplay($name)."</a> (".pnVarPrepForDisplay($email).")</font>\n<font class=\"pn-sub\"> "._ON." ".pnVarPrepForDisplay($datetime)."</font>";
        } else {
            echo "<font class=\"pn-title\">".pnVarPrepForDisplay($subject)."</font>\n";
            if(!$noshowscore && $modoption ) {
                echo "<font class=\"pn-normal\">("._SCORE." $score";
                if($reason>0) echo ", ".pnVarPrepForDisplay($reasons[$reason])."";
                echo ")</font>\n";
            }
            echo "<font class=\"pn-normal\"><br>\n"._BY." ".pnVarPrepForDisplay($name)." "._ON." ".pnVarPrepForDisplay($datetime)."</font>";
        }
        ...
        if(eregi("http://",$url)) {
        echo "<font class=\"pn-normal\"><a class=\"pn-normal\" href=\"$url\" target=\"window\">".pnVarPrepForDisplay($url)."</a></font>\n ";
    }
...
// if not there, a warnin on line 654.
    ...
    echo "<font class=\"pn-title\">" . pnVarPrepHTMLDisplay($subject) . "</font><font class=\"pn-normal\">";
    if(!isset($temp_comment)) {
    echo " ("._SCORE." ".pnVarPrepForDisplay($score).")";
    }
    if($email) {
        echo "<br>"._BY." <a class=\"pn-normal\" href=\"mailto:$email\">".pnVarPrepForDisplay($name)."</a> <font class=\"pn-normal\">(".pnVarPrepForDisplay($email).")</font><font class=\"pn-sub\"> "._ON." ".pnVarPrepForDisplay($date)."</font>";
    } else {
        echo "<br><font class=\"pn-sub\">"._BY." ".pnVarPrepForDisplay($name)." "._ON." ".pnVarPrepForDisplay($date)."</font>";
    }
