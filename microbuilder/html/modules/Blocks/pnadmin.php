<?php
// $Id: pnadmin.php,v 1.1 2004/03/01 10:08:59 mbertier Exp $
// ----------------------------------------------------------------------
...
function blocks_admin_view()
{
    ...
    while(list($bid, $bkey, $title, $url, $position, $weight, $mid, $active, $blanguage) = $result->fields) {
        ...
        $output->SetInputMode(_PNH_VERBATIMINPUT);
        $up = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks',
                                    'admin',
                                    'inc',
                                    array('bid' => $bid,
                                          'authid' => $authid))),
                                    '<img src="images/global/up.gif" alt="' . _UP . '" border="0">');
        $down = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks',
                                      'admin',
                                      'dec',
                                      array('bid' => $bid,
                                            'authid' => $authid))),
        ...
        if ($active) {
            $state = _ACTIVE;
            $options[] = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks',
                                               'admin',
                                               'deactivate',
                                               array('bid' => $bid,
                                                     'authid' => $authid))),
...
function blocks_adminmenu() {
    ...
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $columns = array();
    $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks', 'admin', 'new')), _NEWBLOCK);
    $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks', 'admin', 'view')), _VIEWBLOCKS);
    if (pnSessionGetVar('blocks_show_all')) {
        $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks', 'admin', 'showactive')), _SHOWACTIVEBLOCKS);
    } else {
        $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks', 'admin', 'showall')), _SHOWALLBLOCKS);
    }
    $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Blocks', 'admin', 'config')), _CONFIGURE);
