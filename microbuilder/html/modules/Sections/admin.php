<?php
// File: $Id: admin.php,v 1.1 2004/03/01 10:09:00 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
function sections()
{
    ...
    if (!$result->EOF) {
        ...
        if (pnSecAuthAction(0, 'Sections::Section', '::', ACCESS_EDIT)) {
            OpenTable();
            echo "<center><font class=\"pn-normal\"><b>"._ACTIVESECTIONS."</b>&nbsp;"._CLICK2EDITSEC."</font></center><br>"
            ...
            while(list($secid, $secname) = $result->fields) {
                if (pnSecAuthAction(0, 'Sections::Section', "$secname::$secid", ACCESS_EDIT)) {
                    echo "<font class=\"pn-normal\"><strong><big>&middot;</big></strong>&nbsp;&nbsp;<a href=\"admin.php?module=$GLOBALS[ModName]&amp;op=sectionedit&amp;secid=$secid\">".pnVarPrepForDisplay($secname)."</a></font><br />";
                }
                $result->MoveNext();
            }
            echo "</td></tr></table>";
        ...
        if (pnSecAuthAction(0, 'Sections::Article', ":$secname:", ACCESS_ADD)) {
            ...
            echo "<center><font class=\"pn-title\"><b>"._ADDSECARTICLE."</b></font></center><br>"
                ...
                ."<b><font class=\"pn-normal\">"._TITLE.":</font></b>&nbsp;"
                ."<input type=\"text\" name=\"title\" size=\"60\"><br><br>"
                ."<b><font class=\"pn-normal\">"._SELSECTION.":</font></b><br>";
            $column = &$pntable['sections_column'];
            $result = $dbconn->Execute("SELECT $column[secid], $column[secname]
                                      FROM $pntable[sections] ORDER BY $column[secid]");
            ...
            while(list($secid, $secname) = $result->fields) {
                if (pnSecAuthAction(0, 'Sections::Section', "$secname::$secid", ACCESS_ADD)) {
                    echo "<input type=\"radio\" name=\"secid\" value=\"$secid\"><font class=\"pn-normal\"> ".pnVarPrepForDisplay($secname)."</font><br>";
                }
                $result->MoveNext();
            }
            echo "<font class=\"pn-normal\">"._DONTSELECT."</font><br>";
            echo "<br><br><b><font class=\"pn-normal\">"._LANGUAGE.": </font></b>"
            ...
            asort($langlist);
                        ...
                        echo '</select>';
                        echo "<br><br><b><font class=\"pn-normal\">"._CONTENT.":</font></b><br>"
        ...
        if (pnSecAuthAction(0, 'Sections::Article', '::', ACCESS_EDIT)) {
            ...
            while(list($artid, $secid, $title, $content, $slanguage) = $result->fields) {
                ...
                if (pnSecAuthAction(0, 'Sections::Article', "$title:$secname:$artid", ACCESS_EDIT)) {
                    echo "<li><font class=\"pn-normal\">$title - ($slanguage) - ($secname) [ <a href=\"admin.php?module=$GLOBALS[ModName]&amp;op=secartedit&amp;artid=$artid&amp;authid=".pnSecGenAuthKey()."\">"._EDIT."</a> ";
            ...
            echo "<form action=\"admin.php\" method=\"post\"><font class=\"pn-normal\">"
    ...
    if (pnSecAuthAction(0, 'Sections::Section', "::", ACCESS_ADD)) {
        ...
        echo "<center><font class=\"pn-title\"><b>"._ADDSECTION."</b></font></center><br>"
            ...
            ."<b><font class=\"pn-normal\">"._SECTIONNAME.":&nbsp</font></b>"
...
function secartedit()
{
    ...
    echo "<center><font class=\"pn-title\"><b>"._EDITARTICLE."</b></font></center><br>"
        ."<form action=\"admin.php\" method=\"post\">"
        ."<b><font class=\"pn-normal\">"._TITLE.":</font></b><br>"
    ...
    while(list($secid2, $secname) = $result2->fields) {
        ...
        echo "<input type=\"radio\" name=\"secid\" value=\"$secid2\" $che><font class=\"pn-normal\">".pnVarPrepForDisplay($secname)."</font><br>";
        $che = "";
        $result2->MoveNext();
    }
    echo "<br><br><font class=\"pn-normal\"><b>"._LANGUAGE.": </font></b>" /* ML added dropdown , currentlang is pre-selected */
                ."<select name=\"slanguage\" class=\"pn-text\">";
    $lang = languagelist();
...
function sectionedit()
{
    ...
    echo "<img src=\"images/sections/$image\" border=\"0\" alt=\"\"><br><br>"
        ...
        ."<br><font class=\"pn-normal\">("._SECTIONHAS." $number "._ARTICLESATTACH.")</font>"
        ."<br><br>"
    ...
    $result = $dbconn->Execute("SELECT $column[artid], $column[title]
                              FROM $pntable[seccont]
                              WHERE $column[secid]='".pnVarPrepForStore($secid)."' ORDER BY '".pnVarPrepForStore($column['artid'])."'");
    ...
    echo "</select>&nbsp;&nbsp;"
        ...
        ."</form><br>"
        ."<form action=\"admin.php\" method=\"post\">"
        ."<b><font class=\"pn-normal\">"._SECTIONNAME."</font></b><br><font class=\"pn-sub\">"._40CHARSMAX."</font><br>"
        ."<input type=\"text\" name=\"secname\" size=\"40\" maxlength=\"40\" value=\"$secname\"><br><br>"
        ."<b><font class=\"pn-normal\">"._SECTIONIMG."</font></b><br><font class=\"pn-sub\">"._SECIMGEXAMPLE."</font><br>"
    ...
    if (pnSecAuthAction(0, 'Sections::Section', "$secname::$secid", ACCESS_DELETE)) {
        echo "<font class=\"pn-normal\"> [ <a href=\"admin.php?module=$GLOBALS[ModName]&op=sectiondelete&amp;secid=$secid&amp;ok=0\">"._DELETE."</a> ]</font>";
    }
    echo "</form>";
    CloseTable();
...
function sectiondelete()
{
    ...
    if($ok==1) {
        ...
        list($secname) = $result->fields;
        OpenTable();
        echo "<center><b><font class=\"pn-normal\">"._DELSECTION.": $secname</font></b><br><br>\n";
        echo "<table><tr><td>\n";
...
function secartdelete()
{
    ...
    if($ok == 1) {
        ...
        OpenTable();
        echo "<center><b><font class=\"pn-normal\">"._DELARTICLE.": ".pnVarPrepForDisplay($title)."</font></b><br><br>\n";
        echo "<table><tr><td>\n";
        echo myTextForm("admin.php?module=".$GLOBALS['module']."&op=sections", _NO);
        echo "</td><td>\n";
