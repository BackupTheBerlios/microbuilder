<?php
// File: $Id: article.php,v 1.1 2004/03/01 10:08:58 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
list ($save, $op, $mode, $order, $thold, $sid, $tid) = pnVarCleanFromInput('save', 'op', 'mode', 'order', 'thold', 'sid', 'tid');
...
if (isset($save) &&  (pnUserLoggedIn())) {
    ...
    $sql .= "$column[uorder]='" . pnVarPrepForStore($order) . "', ";
    $sql .= "$column[thold]='" . pnVarPrepForStore($thold) . "'";
    $sql .= " WHERE $column[uid]='" . pnVarPrepForStore($uid) . "'";
    $dbconn->Execute($sql);
}
...
$results = getArticles("{$pntable['stories_column']['sid']}='" . pnVarPrepForStore($sid) . "'", "", "");
