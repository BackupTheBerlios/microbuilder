<?php
// File: $Id: index.php,v 1.1 2004/03/01 10:08:59 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
if ((isset($GLOBALS['topic']) && !empty($GLOBALS['topic']) && !is_numeric($GLOBALS['topic'])) or 
	(isset($GLOBALS['catid']) && !empty($GLOBALS['catid']) && !is_numeric($GLOBALS['catid'])) or
	(isset($GLOBALS['allstories']) && !empty($GLOBALS['allstories']) && !is_numeric($GLOBALS['allstories']))    ) 
{
	include 'header.php';
	OpenTable();
	echo _MODARGSERROR;
...
// end furbo hack stop
	...
	if (isset($topic) && !empty($topic) && is_numeric($topic)) {
		...
		$result = $dbconn->Execute("SELECT $column[topicid], $column[topicname], " .
								...
								"$pntable[topics] WHERE $column[topicid]='".pnVarPrepForStore($topic)."'"); 
		list($topicid, $topicname, $topicimage, $topictext) = $result->fields; 
		echo "<center>";
		if (!empty($topicimage)) {
			echo "<img src=\"".pnConfigGetVar('tipath')."$topicimage\"" . "border=\"0\" alt=\"\"><br /><br />";
		}
		echo "<font class=\"pn-title\">" . _THISISTOPIC . pnVarPrepForDisplay($topictext) . "<br />" . _FOLLOWINGNEWS . "</font></center><br /><br />"; 

		CloseTable(); 
	} else if (!empty($catid) && $catid > 0 && is_numeric($catid)) {
		OpenTable();
		$column = &$pntable['stories_cat_column'];
		$result = $dbconn->Execute("SELECT $column[title] FROM $pntable[stories_cat]
									WHERE $column[catid]=".(int)pnVarPrepForStore($catid)."");
		list($title) = $result->fields;
		echo "<center><br /><font class=\"pn-title\">" . _THISISCATEGORY. pnVarPrepForDisplay($title) . "<br />" . _FOLLOWINGNEWSCATEGORY . "</font></center><br /><br />";
		CloseTable();
	}
...
// end  eugeniobaldi 20020310
    ...
    if (!empty($catid) && !empty($topic)) { // show only one category and one topic
        ...
        $whereclause = "$topiccol[topicid]='".pnVarPrepForStore($topic)."' AND $storcol[catid]='".pnVarPrepForStore($catid)."' ";
        ...
        $whereclause = "$storcol[catid]='".pnVarPrepForStore($catid)."' ";
