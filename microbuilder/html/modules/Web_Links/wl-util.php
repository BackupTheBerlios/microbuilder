<?php
// File: $Id: wl-util.php,v 1.1 2004/03/01 10:09:03 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
function editoriallink($lid) {
    ...
    $result = $dbconn->Execute("SELECT count(*)
                                ...
                                WHERE $column[linkid]='".pnVarPrepForStore($lid)."'");
    list($count) = $result->fields;
    return $count;
}
...
function LinksBottomMenu($lid, $transfertitle, $totalvotes, $totalcomments){
	...
	if (pnSecAuthAction(0, 'Web Links::Category', '::', ACCESS_COMMENT) || pnSecAuthAction(0, 'Web Links::Link', '::$lid', ACCESS_COMMENT)) {
        echo "<a class=\"pn-normal\" href=\"".$GLOBALS['modurl']."&amp;req=ratelink&amp;lid=".pnVarPrepForDisplay($lid)."&amp;ttitle=".pnVarPrepForDisplay($transfertitle)."\">"._RATESITE."</a>";
        echo " | <a class=\"pn-normal\" href=\"".$GLOBALS['modurl']."&amp;req=modifylinkrequest&amp;lid=".pnVarPrepForDisplay($lid)."\">"._MODIFY."</a> | ";
    }
    ...
    if (pnSecAuthAction(0, 'Web Links::Category', '::', ACCESS_READ) || pnSecAuthAction(0, 'Web Links::Link', '::$lid', ACCESS_READ)) {
		...
		// is never called - markwest
     	//if ($totalvotes != 0) {
			echo " | <a href=\"".$GLOBALS['modurl']."&amp;req=viewlinkdetails&amp;lid=".pnVarPrepForDisplay($lid)."&amp;ttitle=".pnVarPrepForDisplay($transfertitle)."\">"._DETAILS."</a>";
        //}
    }
    if (pnSecAuthAction(0, 'Web Links::Category', '::', ACCESS_READ) || pnSecAuthAction(0, 'Web Links::Link', '::$lid', ACCESS_READ)) {
        if ($totalcomments != 0) {
           echo " | <a href=\"".$GLOBALS['modurl']."&amp;req=viewlinkcomments&amp;lid=".pnVarPrepForDisplay($lid)."&amp;ttitle=".pnVarPrepForDisplay($transfertitle)."\">"._COMMENTS." (".pnVarPrepForDisplay($totalcomments).")</a>";
        }
    }
}
...
function web_links_rateMakeStar($score, $max_score)
{
    ...
    for ($i=1;$i<=$max_score;$i++){
        if ($i<=$full_stars){
            $star_line.="<img src='".$rateImgFull."' border='0' alt=''>";
        }elseif ($i<=($half_stars+$full_stars)){
            $star_line.="<img src='".$rateImgHalf."' border='0' alt=''>";
        }elseif ($i<=$max_score){
            $star_line.="<img src='".$rateImgNone."' border='0' alt=''>";
        }
    }
    return $star_line;
}
...
function linkfooter($lid,$ttitle) {
    echo "<font class=\"pn-normal\">[ <a class=\"pn-normal\" href=\"".$GLOBALS['modurl']."&amp;req=visit&amp;lid=".pnVarPrepForDisplay($lid)."\" target=\"_blank\">"._VISITTHISSITE."</a>";
	if (pnSecAuthAction(0, 'Web Links::Category', '::', ACCESS_COMMENT) || pnSecAuthAction(0, 'Web Links::Link', '::$lid', ACCESS_COMMENT)) {
		echo " | <a class=\"pn-normal\" href=\"".$GLOBALS['modurl']."&amp;req=ratelink&amp;lid=".pnVarPrepForDisplay($lid)."&amp;ttitle=".pnVarPrepForDisplay($ttitle)."\">"._RATETHISSITE."</a>";
	}
 	echo "]</font><br /><br />";
    linkfooterchild($lid);
}
