<?php
// File: $Id: wl-newlinks.php,v 1.1 2004/03/01 10:09:03 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
function NewLinks($newlinkshowdays) {
    ...
    while ($counter <= 7-1){
    	...
    	$result=$dbconn->Execute("SELECT $column[cat_id], $column[title] FROM $pntable[links_links] WHERE $column[date] LIKE '%$newlinkDB%'");
...
function NewLinksDate($selectdate) {
    ...
    while(list($lid, $cid, $title, $description, $time, $hits, $linkratingsummary, $totalvotes, $totalcomments)=$result->fields) {
    	...
    	if (pnSecAuthAction(0, "Web Links::Category", "$title::$cid", ACCESS_READ)) {
    		...
    		popgraphic($hits);
    		echo "<br />".pnVarPrepHTMLDisplay($description)."<br />";
