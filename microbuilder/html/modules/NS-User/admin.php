...
function displayUsers()
{
    ...
    if (pnSecAuthAction(0, 'Users::', '::', ACCESS_EDIT)) {
        ...
        echo "<center><font class=\"pn-title\"><b>" . _EDITUSER . "</b></font><br><br>" 
			 ...
			 . "<b><font class=\"pn-normal\">" . _NICKNAME . ": </font></b>"
			 . "<input type=\"text\" name=\"chng_uid\" size=\"20\">\n" 
			 . "<select name=\"op\"><option value=\"modifyUser\">" . _MODIFY . "</option>\n" 
    ...
    if (pnSecAuthAction(0, 'Users::', '::', ACCESS_ADD)) {
		...
		. "<tr><td width=\"100\"><font class=\"pn-normal\">" . _NICKNAME . "</font></td>" 
		...
		. "<tr><td><font class=\"pn-normal\">" . _PASSWORD . "</font></td>" 
		. "<td><input type=\"password\" name=\"add_pass\" size=\"12\" maxlength=\"12\"> <font class=\"pn-sub\">" . _REQUIRED . "</font>" 
		. "<input type=\"hidden\" name=\"add_avatar\" value=\"blank.gif\">" 
		. "<input type=\"hidden\" name=\"module\" value=\"NS-User\">" 
		. "<input type=\"hidden\" name=\"op\" value=\"addUser\">" 
		. "<input type=\"hidden\" name=\"authid\" value=\"" . pnSecGenAuthKey() . "\">" 
		. "</td></tr><tr><td><input type=\"submit\" value=\"" . _ADDUSERBUT . "\"></td></tr></table></form></center>";
        CloseTable();
    } 
    ...
    if (pnSecAuthAction(0, 'Users::', '::', ACCESS_ADD)) {
        ...
        echo "<center><a href=\"admin.php?module=" . $GLOBALS['module'] . "&amp;op=getRegConfig\">" . _REGCONF . "</a></center>";
...
function updateUser()
{
    ...
    if ((!$chng_uname) || !(ereg("^[[:print:]]+", $chng_uname) && !ereg("[[:space:]]", $chng_uname))) {
        ...
        OpenTable();
        echo "<center><font class=\"pn-normal\">" . _ERRORINVNICK . "<br><br>" . "" . _GOBACK . "</font></center>";
        CloseTable();
        include("footer.php");
        exit;
    } 
    ...
    if ($chng_url != "") {
        ...
        if ($url_array[0] != "http") {
            ...
            OpenTable();
            echo "<center><font class=\"pn-normal\">" . _ERRORINVURL . "<br><br>" . "" . _GOBACK . "</font></center>";
            CloseTable();
            include("footer.php");
            exit;
        } 
    ...
    if ($chng_pass2 != "") {
        if ($chng_pass != $chng_pass2) {
            ...
            OpenTable();
            echo "<center><font class=\"pn-normal\">" . _PASSWDNOMATCH . "<br><br>" . "" . _GOBACK . "</font></center>";
            CloseTable();
            include("footer.php");
            exit;
        } 
...
function deleteUser($chng_uid)
{
    ...
    echo "<font class=\"pn-title\">" . _SURE2DELETE . " " . pnVarPrepForDisplay(stripslashes($chng_uid)) . "?<br><br>" . "[ <a href=\"admin.php?module=NS-User&amp;op=delUserConf&amp;del_uid=$uid&amp;authid=$authid\">" . _YES . "</a> | <a href=\"admin.php?module=NS-User&amp;op=mod_users\">" . _NO . "</a> ]</font></center>";
    CloseTable();
    include("footer.php");
} 
...
function user_admin_getConfig()
{
    ...
    GraphicAdmin();
    OpenTable();
    print '<center><font size="3" class="pn-title"><b>' . _USERCONF . '</b></font></center><br>' . '<form action="admin.php" method="post">' . '<table border="0"><tr><td class="pn-normal">' . _USERPATH . "</td><td><input type=\"text\" name=\"xuserimg\" value=\"" . pnConfigGetVar('userimg') . "\" size=\"50\" class=\"pn-normal\">" . '</td></tr><tr><td class="pn-normal">' . _USERGRAPHIC . '</td><td class="pn-normal">' . "<input type=\"radio\" name=\"xusergraphic\" value=\"1\" class=\"pn-normal\"" . $sel_usergraphic['1'] . ">" . _YES . ' &nbsp;' . "<input type=\"radio\" name=\"xusergraphic\" value=\"0\" class=\"pn-normal\"" . $sel_usergraphic['0'] . ">" . _NO . '</td></tr></table>' . "<input type=\"hidden\" name=\"module\" value=\"" . $GLOBALS['module'] . "\">" . "<input type=\"hidden\" name=\"op\" value=\"setConfig\">" . "<input type=\"hidden\" name=\"authid\" value=\"" . pnSecGenAuthKey() . "\">" . "<input type=\"submit\" value=\"" . _SUBMIT . "\">" . "</form>";
    CloseTable();
    include ("footer.php");
} 
...
function user_dynamic_data()
{
    ...
    OpenTable();
    print '<center><font size="3" class="pn-title"><b>' . _DYNAMICDATA . '</b></font></center><br>' 
	    . '<table class=\'pn-normal\' border=\'1\' width=\'100%\'>' . '<tr>' . '<th>' . _FIELDACTIVE . '</th>' 
    ...
    while (list($prop_id, $prop_label, $prop_dtype, $prop_length, $prop_weight, $prop_validation) = $result->fields) {
        ...
        switch (true) {
            ...
            case ($prop_weight <> 0):
                $img_cmd = "<a href=\"admin.php?module=" . $GLOBALS['module'] . "&amp;op=deactivate_property&amp;property=$prop_id&amp;weight=$prop_weight\">" 
				         . '<img src="images/global/green_dot.gif" border=0 ALT="' . _FIELD_DEACTIVATE . '">' . '</a>';
                break;
            default:
                $img_cmd = "<a href=\"admin.php?module=" . $GLOBALS['module'] . "&amp;op=activate_property&amp;property=$prop_id&amp;weight=$prop_weight\">" 
				         . '<img src="images/global/red_dot.gif" border=0 ALT="' . _FIELD_ACTIVATE . '">' . '</a>';
        } 
        ...
        switch (true) {
            ...
            case ($active_count == 1):
                $arrows = "<a href=\"admin.php?module=" . $GLOBALS['module'] . "&amp;op=increase_weight&amp;property=$prop_id&amp;weight=$prop_weight\">" 
				        . '<img src="images/global/down.gif" alt="' . _DOWN . '" border="0">' . '</a>';
                break;
        ...
        print '<tr><td width=\'5%\' align=\'center\'>' . "$img_cmd" . '</td>' . '<td width=\'12%\'>' . $prop_label . '</td>' 
		    . '<td width=\'12%\'>' . $prop_label_text . '</td>' . '<td width=\'10%\' align=\'center\'>' . $arrows . '</td>' 
			. '<td width=\'15%\' align=\'center\'>' . $data_type_text . '</td>' . '<td width=\'10%\' align=\'center\'>' . $data_length_text . '</td>' 
    ...
    print '<center><font size="3" class="pn-title"><b>' . _ADDFIELD . '</b></font></center><br>' . '<form action="admin.php" method="post">' 
	    . '<table class=\'pn-normal\'>' . '<tr>' . '<th align=\'left\'>' . _FIELDLABEL . ':</th>' 
		. '<td>' . '<input type="text" name="label" value="" size="20" maxlength="20" class="pn-normal">' . '&nbsp;' . _ADDINSTRUCTIONS . '</td>' . '</tr>' 
		. '<tr>' . '<th align=\'left\'>' . _FIELDTYPE . ':</th>' . '<td>' . '<select name="dtype" class="pn-normal">' . '<option value="' . _UDCONST_STRING . '">' 
		. _UDT_STRING . '</option>' . "\n" . '<option value="' . _UDCONST_TEXT . '">' . _UDT_TEXT . '</option>' . "\n" . '<option value="' . _UDCONST_FLOAT . '">' 
		. _UDT_FLOAT . '</option>' . "\n" . '<option value="' . _UDCONST_INTEGER . '">' . _UDT_INTEGER . '</option>' . "\n" . '</select>' . '</td>' . '</tr>' 
		. '<tr>' . '<th align=\'left\'>' . _FIELDLENGTH . ':</th>' . '<td>' . '<input type="text" name="prop_len" value="" size="3" maxlength="3" class="pn-normal">' 
		. '&nbsp;' . _STRING_INSTRUCTIONS . '</td>' . '</tr>' . '<tr><td></td><td>' . "<input type=\"hidden\" name=\"module\" value=\"" . $GLOBALS['module'] . "\">" 
		. "<input type=\"hidden\" name=\"op\" value=\"addDynamic\">" . "<input type=\"submit\" value=\"" . _SUBMIT . "\">" . '</td></tr>' . '</table>' 
		. '<input type="hidden" name="prop_weight" value="' . $next_prop_weight . '">' . '<input type="hidden" name="validation" value="">' 
		. "<input type=\"hidden\" name=\"authid\" value=\"" . pnSecGenAuthKey() . "\">" . '<input type="hidden" name="op" value="add_property">' . '</form>';
    CloseTable();
    include ("footer.php");
} 
...
function delete_property($var)
{
    ...
    echo "" . _FIELD_DEL_SURE . " " . pnVarPrepForDisplay($plabel) . "?<br><br>" . "[ <a href=\"admin.php?module=" . $GLOBALS['module'] . "&amp;op=delPropConf&amp;label=" . $plabel . "\">" . _YES . "</a> | <a href=\"admin.php?module=" . $GLOBALS['module'] . "&amp;op=getDynamic\">" . _NO . "</a> ]</center>";
    CloseTable();
    include("footer.php");
} 
...
function user_admin_getRegConfig()
{
    ...
    GraphicAdmin();
    OpenTable();
    print '<center><font size="3" class="pn-title"><b>' . _REGCONF . '</b></font></center><br>' 
	    . '<form action="admin.php" method="post">' 
		. '<table border="0"><tr><td class="pn-normal">' . _UNIEMAIL . '</td><td class="pn-normal">' 
		. "<input type=\"radio\" name=\"xreg_uniemail\" value=\"1\" class=\"pn-normal\"" . $sel_reg_uniemail['1'] . ">" . _YES . ' &nbsp;' 
		. "<input type=\"radio\" name=\"xreg_uniemail\" value=\"0\" class=\"pn-normal\"" . $sel_reg_uniemail['0'] . ">" . _NO . " &nbsp;" . _UNIEMAILDESC . "\n" 
		. '</td></tr><tr><td class="pn-normal">' . _OPTITEMS . '</td><td class="pn-normal">' 
		. "<input type=\"radio\" name=\"xreg_optitems\" value=\"1\" class=\"pn-normal\"" . $sel_reg_optitems['1'] . ">" . _YES . ' &nbsp;'
		 . "<input type=\"radio\" name=\"xreg_optitems\" value=\"0\" class=\"pn-normal\"" . $sel_reg_optitems['0'] . ">" . _NO . " &nbsp;" . _OPTITEMSDESC . "\n" 
		 . '</td></tr><tr><td class="pn-normal" valign="top">' . _ALLOWREG . ' </td><td class="pn-normal">' 
		 . "<input type=\"radio\" name=\"xreg_allowreg\" value=\"1\" class=\"pn-normal\"" . $sel_reg_allowreg['1'] . ">" . _YES . ' &nbsp;' 
		 . "<input type=\"radio\" name=\"xreg_allowreg\" value=\"0\" class=\"pn-normal\"" . $sel_reg_allowreg['0'] . ">" . _NO . " &nbsp;<br>\n" . _IFNO 
		 . "<br>&nbsp;&nbsp;&nbsp;<textarea name=\"xreg_noregreasons\" cols=\"80\" rows=\"10\">" . pnConfigGetVar('reg_noregreasons') . "</textarea>" 
		 . '</td></tr><tr><td class="pn-normal">' . _VERIFYEMAIL . '</td><td class="pn-normal">' 
		 . "<input type=\"radio\" name=\"xreg_verifyemail\" value=\"1\" class=\"pn-normal\"" . $sel_reg_verifyemail['1'] . ">" . _YES . ' &nbsp;' 
		 . "<input type=\"radio\" name=\"xreg_verifyemail\" value=\"0\" class=\"pn-normal\"" . $sel_reg_verifyemail['0'] . ">" . _NO . " &nbsp;" . _PASSBYMAIL . "\n" 
		 . "</td></tr><tr><td class=\"pn-normal\">" . _NOTIFYEMAIL . "</td><td class=\"pn-normal\">"
		 . "<input type=\"text\" name=\"xreg_notifyemail\" value=\"" . pnConfigGetVar('reg_notifyemail') . "\" size=\"20\" maxlength=\"200\" class=\"pn-normal\">" . _NOTIFYEMAILDESC . "\n" 
		 . "</td></tr><tr><td class=\"pn-normal\">" . _ILLEGALUNAME . "</td><td class=\"pn-normal\">" 
		 . "<input type=\"text\" name=\"xreg_Illegalusername\" value=\"" . pnConfigGetVar('reg_Illegalusername') . "\" size=\"20\" class=\"pn-normal\">" . _ILLEGALUNAMEDESC . "\n" 
		 . '</td></tr><tr><td class="pn-normal">' . _MINAGE . "</td><td class=\"pn-normal\"><input type=\"text\" name=\"xminage\" value=\"" 
		 . pnConfigGetVar('minage') . "\" size=\"2\" maxlength=\"2\" class=\"pn-normal\"> " . _MINAGEDESCR . "\n" 
		 . '</td></tr><tr><td class="pn-normal">' . _PASSWDLEN . '</td><td>' . '<select name="xminpass" size="1" class="pn-normal">' 
		 ...
		 . '</select>' . '</td></tr></table>' . "<input type=\"hidden\" name=\"module\" value=\"" . $GLOBALS['module'] . "\">" 
		 . "<input type=\"hidden\" name=\"op\" value=\"setRegConfig\">" . "<input type=\"hidden\" name=\"authid\" value=\"" . pnSecGenAuthKey() . "\">" 
		 . "<input type=\"submit\" value=\"" . _SUBMIT . "\">" . "</form>";
    CloseTable();
    include ("footer.php");
} 

function user_admin_setRegConfig($var)
{
