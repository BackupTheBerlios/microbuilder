<?php
// File: $Id: pnadmin.php,v 1.1 2004/03/01 10:09:05 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
        ...
        if (pnSecAuthAction(0, 'Modules::', "$mod[name]::$mod[id]", ACCESS_ADMIN)) {
            ...
            switch ($mod['state']) {
                case _PNMODULE_STATE_ACTIVE:
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'deactivate', array('id' => $mod['id'], 'authid' => $authid))), _DEACTIVATE);
                    $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'regenerate')), _GENERATEMODS);
                    break;
                case _PNMODULE_STATE_INACTIVE:
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'activate', array('id' => $mod['id'], 'authid' => $authid))), _ACTIVATE);
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'remove', array('id' => $mod['id'], 'authid' => $authid))), _REMOVE);
                    break;
                case _PNMODULE_STATE_MISSING:
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'remove', array('id' => $mod['id'], 'authid' => $authid))), _REMOVE);
                    break;
                case _PNMODULE_STATE_UPGRADED:
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'upgrade', array('id' => $mod['id'], 'authid' => $authid))), _UPGRADE);
                    break;
                case _PNMODULE_STATE_UNINITIALISED:
                default:
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'initialise', array('id' => $mod['id'], 'authid' => $authid))), _INITIALISE);
                    $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'remove', array('id' => $mod['id'], 'authid' => $authid))), _REMOVE);
                    break;
            }

            $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Modules', 'admin', 'modify', array('id' => $mod['id']))), _EDIT);
