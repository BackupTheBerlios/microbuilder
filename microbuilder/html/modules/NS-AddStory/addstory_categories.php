<?php
// File: $Id: addstory_categories.php,v 1.1 2004/03/01 10:09:07 mbertier Exp $
// ----------------------------------------------------------------------
...
//        echo "<option name=\"catid\" value=\"0\" $selcat>Articles</option>";
        ...
        while(list($catid, $title) = $selcat->fields) {
            ...
            echo "<option value=\"$catid\">".pnVarPrepForDisplay($title)."</option>";
            $selcat->MoveNext();
        }
        echo "</select>"
             ."</td></tr><tr><td>&nbsp;</td><td>"
    ...
    if (empty($catid)) {
		...
		while(list($catid, $title) = $selcat->fields) {
			...
			if ($catid == $thiscat) {
				...
				echo "<option value=\"$catid\" $sel>".pnVarPrepForDisplay($title)."</option>";
...
function SelectCategory($cat)
{
    ...
    $column = &$pntable['stories_cat_column'];
	//[ #469 ] Sort categories in NS-AddStory - Rolf Rejek (homestarter)
    $selcat = $dbconn->Execute("SELECT $column[catid], $column[title]
