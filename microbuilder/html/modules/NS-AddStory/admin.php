<?php
// File: $Id: admin.php,v 1.1 2004/03/01 10:09:07 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
/*    if (!pnSecConfirmAuthKey()) {
    ...
    if ($automated == 1) {
        ...
        $result = $dbconn->Execute("DELETE FROM $queuetable
                                  WHERE $queuecolumn[qid]='".pnVarPrepForStore($qid)."'");
        if ($result === false) {
...
function deleteStory($qid)
{
    ...
    $result = $dbconn->Execute("DELETE FROM {$pntable['queue']}
                              WHERE {$pntable['queue_column']['qid']}='".pnVarPrepForStore($qid)."'");
    if ($result === false) {
...
function editStory()
{
    ...
    if ($result->EOF) {
        ...
        CloseTable();
        OpenTable();
        echo "<center><b><font class=\"pn-normal\">"._NOTSUCHARTICLE."</b><br><br>"._GOBACK."</font></center>";
        CloseTable();
        include("footer.php");
        return;
    }
    ...
    $result = $dbconn->Execute("SELECT {$catcolumn['title']}
                              FROM {$pntable['stories_cat']}
                              WHERE {$catcolumn['catid']} = '".pnVarPrepForStore($catid)."'");
    ...
    if (pnSecAuthAction(0, 'Stories::Story', "$aid:$cattitle:$sid", ACCESS_EDIT)) {
        ...
        OpenTable();
        echo "<center><b><font class=\"pn-normal\">"._NOTAUTHORIZED1."</b><br><br>"
            .""._GOBACK."</font></center>";
        CloseTable();
        include("footer.php");
    }
...
function removeStory($sid, $ok=0)
{
    ...
    if (pnSecAuthAction(0, 'Stories::Story', "$aid:$cattitle:$sid", ACCESS_DELETE)) {
        ...
        if (!pnSecConfirmAuthKey()) {
            ...
            OpenTable();
            echo "<center><font class=\"pn-normal\">"._REMOVESTORY."<b> $sid - $stitle</b> - "._ANDCOMMENTS."</font>";
            echo "<table><tr><td>\n";
    ...
    if (pnSecAuthAction(0, 'Stories::Story', "$aid:$cattitle:$anid", ACCESS_DELETE)) {
        if($ok && csssafe("postnuke")) {
            $result = $dbconn->Execute("DELETE FROM {$pntable['autonews']}
                                      WHERE {$pntable['autonews_column']['anid']}='".pnVarPrepForStore($anid)."'");
            if ($result === false) {
            ...
            $result = $dbconn->Execute("UPDATE {$pntable['users']}
                                      SET {$column['counter']} = {$column['counter']} - 1
                                      WHERE {$column['uid']}='".pnVarPrepForStore($aid)."'");
            if ($result === false) {
                error_log("stories->autodelete: " . $dbconn->ErrorMsg());
                PN_DBMsgError($dbconn, __FILE__, __LINE__, "stories->autodelete: Error accessing the database");
            }
            ...
            OpenTable();
            echo "<center><font class=\"pn-normal\">"._REMOVEAUTOSTORY."<b> $anid - $titlean</font></b>";
            echo "<table><tr><td>\n";
            echo myTextForm('admin.php', _NO);
            echo "</td><td>\n";
...
function autoEdit($anid)
{
    ...
    $result = $dbconn->Execute("SELECT $ancolumn[title], $sccolumn[title], $sccolumn[catid]
                               ...
                               WHERE $ancolumn[anid]='".pnVarPrepForStore($anid)."'
...
function submissions()
{
    ...
    if($result->EOF) {
        echo "<table width=\"100%\"><tr><td align=\"center\"><font class=\"pn-normal\"><b>"
        ._NOSUBMISSIONS."</b> [ <a href=\"admin.php?module=".$module."&amp;op=ListArchive\">"._ARCHIVESUBS."</a> ]</font></td></tr></table>\n";
    } else {
        echo "<center><font class=\"pn-normal\"><b>"._NEWSUBMISSIONS
        ."</b> [ <a href=\"admin.php?module=".$module."&amp;op=ListArchive\">"
        ._ARCHIVESUBS."</a> ]</font></center><table width=\"100%\" border=\"1\" >\n";
...
//                    }
        ...
        if ($dummy < 1) {
        echo "<tr><td align=\"center\"><b><font class=\"pn-normal\">"._NOSUBMISSIONS."</font></b></form></td></tr></table>\n";
        } else {
        echo "</table>\n";
        }
    }
...
function ArchiveStory($qid)
{
    ...
    $result = $dbconn->Execute("SELECT {$column['subject']}
                              FROM {$pntable['queue']}
                              WHERE {$column['qid']}='".pnVarPrepForStore($qid)."'");
    if ($result === false) {
