<?php
// $Id: pnadmin.php,v 1.1 2004/03/01 10:09:01 mbertier Exp $
// ----------------------------------------------------------------------
...
function autolinks_admin_modify($args)
{
    ...
    if (!pnSecAuthAction(0, 'Autolinks::Item', "$keyword::$lid", ACCESS_EDIT)) {
        $output->Text(_AUTOLINKSNOAUTH);
        return $output->GetOutput();
    }
...
function autolinks_admin_delete($args)
{
    ...
    if (empty($confirmation)) {
        ...
        $output->ConfirmAction(_AUTOLINKSCONFIRMDELETE,
                               pnVarPrepForDisplay(pnModURL('Autolinks',
                                        'admin',
                                        'delete')),
    ...
    foreach ($links as $link) {
        ...
        if (pnSecAuthAction(0, 'Autolinks::', "$link[keyword]::$link[lid]", ACCESS_READ)) {
            ...
            if (pnSecAuthAction(0, 'Autolinks::', "$link[keyword]::$link[lid]", ACCESS_EDIT)) {
                $options[] = $output->URL(pnVarPrepForDisplay(pnModURL('Autolinks',
                                                   'admin',
                                                   'modify',
                                                   array('lid' => $link['lid']))),
                                          _EDIT);
...
function autolinks_adminmenu()
{
    ...
    $columns = array();
    $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Autolinks',
                                       'admin',
                                       'new')),
                              _AUTOLINKSADD); 
    $columns[] = $output->URL(pnVarPrepForDisplay(pnModURL('Autolinks',
