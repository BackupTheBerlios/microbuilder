<?php
// File: $Id: pnadmin.php,v 1.1 2004/03/01 10:09:00 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
function quotes_adminmenu()
{
    ...
    $output->LineBreak(1);
    $output->URL(pnVarPrepForDisplay(pnModURL('Quotes', 'admin', 'qotddisplay', array('page' => 1, 'authid' => $authid))), _QOTDMODIFY);
    $output->TableColEnd();
    $output->TableRowEnd();
    $output->TableEnd();
...
function quotes_admin_qotddisplay()
{
	...
	$output->SetOutputMode(_PNH_RETURNOUTPUT);
	    ...
	    if(pnSecAuthAction(0, 'Quotes::', "$quote[author]::$quote[qid]", ACCESS_EDIT)) {
	        $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Quotes', 'admin', 'edit', array('qid' => $quote['qid'], 'qauthor' => urlencode($quote['author']), 'authid' => $authid))), _QOTDEDITACTION);
	    }
	    if(pnSecAuthAction(0, 'Quotes::', "$quote[author]::$quote[qid]", ACCESS_DELETE)) {
	        $actions[] = $output->URL(pnVarPrepForDisplay(pnModURL('Quotes', 'admin', 'delete', array('qid' => $quote['qid'], 'qauthor' => urlencode($quote['author']), 'authid' => $authid))), _QOTDDELETEACTION);
	    }
	$output->SetOutputMode(_PNH_KEEPOUTPUT);
...
function quotes_admin_delete()
{
	...
	$output->SetInputMode(_PNH_PARSEINPUT);
        $output->ConfirmAction(_QOTDDEL,
                               ...
                               _CANCELQUOTEDELETE,
                               pnVarPrepForDisplay(pnModURL('Quotes','admin','qotddisplay')), array('qid' => $qid, 'qauthor' => $qauthor));
        return $output->GetOutput();
    }
    if (!pnSecConfirmAuthKey()) {
        pnSessionSetVar('errormsg', _BADAUTHKEY);
