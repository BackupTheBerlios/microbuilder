<?php
// Generated: $d$ by $id$
// ----------------------------------------------------------------------
// POST-NUKE Content Management System
// Copyright (C) 2001 by the Post-Nuke Development Team.
// http://www.postnuke.com/
// ----------------------------------------------------------------------
// Based on:
// PHP-NUKE Web Portal System - http://phpnuke.org/
// Thatware - http://thatware.org/
// ----------------------------------------------------------------------
// LICENSE
//
// This program is free software; you can redistribute it and/or
// modify it under the terms of the GNU General Public License (GPL)
// as published by the Free Software Foundation; either version 2
// of the License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// To read the license please visit http://www.gnu.org/copyleft/gpl.html
// ----------------------------------------------------------------------
// Original Author of file: Everyone
// Purpose of file: Translation files
// Translation team: Read credits in /docs/CREDITS.txt
// ----------------------------------------------------------------------
define('_ABOUTPOSTING','About sending private messages');
define('_ALLREGCANPOST','All registered users can send private messages');
define('_BBCODEDISSABLE','Disable <a href="modules/Messages/bbcode_ref.php" target="_blank"><i>BBCode</i></a> on this post');
define('_BB_CODE','BBCode');
define('_CANCELREPLY','Cancel reply');
define('_CANCELSEND','Cancel send');
define('_CHECKALL','Check all');
define('_CHECKNAMEANDTRY','Please check that the name of the user you are sending a message to exists');
define('_DONTHAVEMESSAGES','You don\'t have any messages');
define('_FROM','From');
define('_HTMLDISSABLE','Disable HTML on this post');
define('_INDEX','Index');
define('_MESSAGE','Message');
define('_MESSAGEICON','Message icon');
define('_MSGDELETED','Your message has been deleted.');
define('_MSGERROR','Sending private message Error');
define('_MSGERROR0005','Error getting messages from the database');
define('_MSGERROR0017','You must type a <b>subject</b> to send the private message. You can\'t use an empty subject. Plesae go back and enter the subject');
define('_MSGERROR0018','You must choose <b>message icon</b> to send the private message. Please go back and choose message icon.');
define('_MSGERROR0019','You must type a <b>message</b> to send the private message. You can\'t send an empty message. Please go back and enter a message.');
define('_MSGERROR0020','Could not enter data into the database. Please go back and try again.');
define('_MSGERROR0021','Can\'t delete the selected message.');
define('_MSGERROR0022','An error ocurred while querying the database.');
define('_MSGERROR0023','Selected message was not found in the database.');
define('_MSGERROR0024','You can\'t reply to that message. It wasn\'t sent to you.');
define('_MSGGOBACK','Go back');
define('_MSGPOSTED','Your message has been sent.');
define('_NEXTMESSAGE','Next message');
define('_NOTREAD','Not read');
define('_NO_MESSAGE_SELECTED','No message(s) selected');
define('_PREVIOUSMESSAGE','Previous message');
define('_PRIVATEMESSAGE','Private message');
define('_PRIVATEMESSAGES','Your private messages');
define('_PROFILE','Profile');
define('_RE','Re');
define('_RETURNTOPMSG','You can click here to reload your private messages.');
define('_SENT','Sent');
define('_SMILEDISSABLE','Disable <a href="modules/Messages/bb_smilies.php" target="_blank"><i>smilies</i></a> on this Post');
define('_USERNOTINDB','Unknown user');
define('_WROTE','wrote');
?>