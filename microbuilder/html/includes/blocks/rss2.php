<?php
// File: $Id: rss2.php,v 1.1 2004/03/01 10:09:12 mbertier Exp $ $Name:  $
// ----------------------------------------------------------------------
...
function blocks_rss2_select($row)
{
    ...
    $output =
    '<tr><td class="pn-normal">RSS File URL:</td><td>'
    ."<input type=\"text\" name=\"url\" size=\"\" maxlength=\"\" value=\"$row[url]\">"
    .'&nbsp;&nbsp;<select name="rssurl" size="1">';

    if ($row['url'])
    {
        $output.= "<option value=\"\" selected>"._CURRENT."<--- Current</option>";
    } else {
...
//    }
    ...
    while (list (, $line) = each($a)) {
        ...
        case ereg($element_tit, $line) :
            ...
            // 23/01/01 devshed utilise des attributs <i> dans la partie titres
        ...
        case ereg($element_des, $line) :
            ...
            $des = ereg_replace("&lt;a", "<b><a", $des);
            $des = ereg_replace("a&gt;", "a></b>", $des);
    ...
    $ligne = " <LI><A href=\"" . $url . "\" target=\"other\">" . $title . "</A>\n";
    ...
    $h['titles'] = '';
    $header = "<center>" .
    //          "<a href='" . $channelurl . "' target=\"other\">" . $channeltit . "</a><br>" .
          "<a href='" . $imgurl . "' target=\"other\">" .
    ...
    if ($c['query']) {
        ...
        $hidden .= ereg_replace("(.*)=(.*)",
        "<input type=\"hidden\" name=\"\\1\" value=\"\\2\">", $arg);
        }
    }
    $footer = "<form  method=\"get\" action=\"" . $cgi .
            "\"  target=\"other\">" . $lib .
            '&nbsp;<INPUT TYPE="text" ' .
            'NAME="' . $input . '" SIZE="10" MAXLENGTH="10"><br>' .
